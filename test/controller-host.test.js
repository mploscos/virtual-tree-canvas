...
test('automatic painting coalesces changes and cancels scheduled work on destroy', () => {
  ...
  const controller = new TreeViewController({ autoRender: true });
  controller.canvas = { ownerDocument: { defaultView: view } };
  controller.render = () => { renders++; };
  controller.setData([{ id: 'one' }]);
  controller.setDynamicState([{ id: 'one', state: { value: 1 } }]);
...
test('icon resolver applies strings and visual patches without mutating caller nodes', () => {
  const nodes = [{ id: 'one' }, { id: 'two', icon: 'folder' }, { id: 'three' }];
  const controller = new TreeViewController();
  controller.setData(nodes, { iconResolver: node => node.id === 'one' ? 'person' : node.id === 'two' ? { icon: 'data-bus' } : null });
